...
/* ──────────────────────────────────────────────────────────────
   Utilitários para JID
   - mantém :device
   - normaliza @c.us -> @s.whatsapp.net
   - não tenta “adivinhar” LID -> JID (isso é papel do wrapper/keystore)
   ────────────────────────────────────────────────────────────── */
const normalizeJidKeepDevice = (jid?: string | null) => {
  if (!jid) return jid as any
  let out = String(jid)
  if (/@c\.us$/i.test(out)) out = out.replace(/@c\.us$/i, '@s.whatsapp.net')
  return out
}
...
const ensureAssertSession = async (
  ...
  const candidates = Array.from(
    new Set(
      jids
        .map(normalizeJidKeepDevice)
        .filter(Boolean) as string[]
    )
  )
  if (!candidates.length) return
  try {
    await assertSessions(candidates, true)
  } catch (e) {
    logger?.warn?.({ candidates }, 'assertSessions(force) failed')
  }
}
...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
  ...
  const sendRetryRequest = async(node: BinaryNode, forceIncludeKeys = false) => {
    ...
    await authState.keys.transaction(
      async() => {
        const toJid = normalizeJidKeepDevice(node.attrs.from) as string
        ...
        // (pt-BR) pré-assert da sessão do destinatário normalizado
        await ensureAssertSession(assertSessions, logger, [
          toJid,
  ...
  const sendMessagesAgain = async(
    ...
    // (pt-BR) garante sessão antes de reenviar
    await ensureAssertSession(assertSessions, logger, [participant])
    ...
    for(const [i, msg] of msgs.entries()) {
      if(msg) {
        updateSendMessageAgainCount(ids[i], participant as string)
              ...
              if(willSendMessageAgain(ids[0], key.participant!)) {
                if(key.fromMe) {
                  try {
                    ...
                    logger.error({ key, ids, trace: (error as any).stack }, 'error in sending message again')
                  }
                } else {
        ...
        processingMutex.mutex(
          async() => {
            ...
            if(msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT) {
              ...
              // (pt-BR) antes do retry, garanta a sessão para from/participant
              await ensureAssertSession(assertSessions, logger, [
                node.attrs.from,
                node.attrs.participant,
                node.attrs.recipient
  ...
  /// processes a node with the given function and buffers if needed
